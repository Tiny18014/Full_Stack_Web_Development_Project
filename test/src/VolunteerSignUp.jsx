...
import './signin.css'; // Assuming the same CSS file is applicable
...
function VolunteerSignUp() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:5137/registerVolunteer', formData)
      .then(result => {
        console.log(result);
        navigate('/volunteerLogin'); // Navigate to a thank you page or dashboard after successful signup
      })
    ...
    <>
      <nav className="navbar" style={{ backgroundColor: '#ffc300' }}>
        <div className="container">
          <a href="https://ibb.co/bLFg6n8" className="logo">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {Object.keys(formData).map(key => (
            <div key={key} className="mb-3">
              ...
              <input
                type={key === 'email' ? 'email' : key === 'password' ? 'password' : 'text'} // Handling input type for password and email fields
                id={key}
                name={key}
                value={formData[key]}
