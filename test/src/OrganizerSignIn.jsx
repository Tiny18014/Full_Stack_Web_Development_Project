...
function OrganizerSignIn({ setUserName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5137/organizerLogin', { email, password });
      const data = response.data;

      // Check the response for a success message
      if (data.message === 'Success') {
        setUserName(data.name);
        navigate('/organizerDashboard'); // Adjust the route as necessary
      } else {
        // Handle non-success messages by setting the error state
      ...
      <footer className="footer-section">
        <div className="container footer-grid">
          <div className="footer-about">
            ...
            <p>© 2004 – 2024 We:Community. All Rights Reserved.</p>
          </div>
          <div className="footer-contact">
            <h3 style={{ color: '#ffc300' }}>Contact:</h3>
            <a href="mailto:info@we-community.co.uk">info@we-community.co.in</a>
